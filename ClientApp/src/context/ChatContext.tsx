...
type Action =
  ...
  | { type: 'UPDATE_REACTIONS'; payload: { msgId: number; reactions: ReactionDto[] } }
...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    lobby.on('UsersOnline', (count: number) => dispatch({ type: 'SET_ONLINE', payload: count }))
    lobby.start().catch(() => {})
    return () => { lobby.stop() }
  }, [])
  ...
  const switchRoom = useCallback(async (id: string, _name: string) => {
    ...
    try { await conn.invoke('LeaveRoom', currentRoomRef.current) } catch {}
    dispatch({ type: 'SET_ROOM', payload: id })
    try { await conn.invoke('JoinRoom', id) } catch {}
  }, [])

  // ── Send message ──────────────────────────────────────────────────────────
  const sendMessage = useCallback(async (text: string, replyId?: number | null) => {
    const conn = chatConnRef.current
    if (!conn) return
    await conn.invoke('SendMessage', currentRoomRef.current, text, replyId ?? null)
  ...
  const startTyping = useCallback(() => {
    const conn = chatConnRef.current
    if (!conn) return
    try { conn.invoke('StartTyping', currentRoomRef.current) } catch {}
    if (typingTimerRef.current) clearTimeout(typingTimerRef.current)
    typingTimerRef.current = setTimeout(() => {
      try { conn.invoke('StopTyping', currentRoomRef.current) } catch {}
    }, 1000)
  }, [])
